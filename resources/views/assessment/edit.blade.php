...
<div id="successMessage" class="mt-3" style="display: none;">
    ...
    <div class="d-flex justify-content-between pt-3">
        <div class="bd-text">
        <h3><a href="javascript:void(0)" class="backlink"><i class="fa fa-long-arrow-left" aria-hidden="true"></i></a> <b>Edit Assessment</b></h3>
        </div>
    ...
    <div class="mt-3">
        <form method="post" id="updateAssessment" enctype="multipart/form-data">
        <input type="hidden" value="<?php if(isset($projectIds[0]->project_id)){ echo $projectIds[0]->project_id;} ?>" name="projectinclude" id="includeProjectArray">
        @csrf
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-md-3 form-group mb-3">
                            ...
                            <input class="form-control" type="text" name="title" value="{{$assessment->title}}">

                                <div class="invalid-feedback" id="title"></div>
                        </div>

                        <div class="col-md-3 form-group mb-3">
                            <label>Assign Project <span class="text-danger"> *</span></label> </label>
                            <select class="select2 form-control" value="" name="project_type" data-placeholder="Assign Project" id="project_type">
                                <option></option>
                                 {{-- @if ($assessment->project_type !="")
                                    <option selected value="{{$assessment->project_type}}">{{$assessment->project_type}}</option>
                                 @endif --}}
                                 <option value="Yes" @if($assessment->project_type == 'Yes') {{__('selected')}} @endif>Yes</option>
                                 <option value="No" @if($assessment->project_type == 'No') {{__('selected')}} @endif>No</option>
                        ...
                        <div class="col-md-3 form-group mb-3">
                            ...
                            <select class="select2 form-control" value="" id="shedule" name="administration_schedule" data-placeholder="" multiple="multiple">
                           <?php    $scheduleUpcoming = explode(',' , $assessment->administration_schedule );
                    ...
                    <div class="row">
                        <div class="mx-auto col-md-12 mb-3" style="height: 100%; width:100%">
                            ...
                            <textarea id="assessmenteditor"  style="width:100%; height:200px;"  name="description">
                              ...
                              {!! old('content', strip_tags($assessment->description)) !!}
                            </textarea>
                                ...
                                @if (!empty($questions))
                                    @foreach ($questions as $question)
                                            ...
                                            @elseif ($question->question_type == 'textType')
                                                <div class="q-html t-html textType" data-id="{{$question->id}}">
                                            ...
                                            <div class="q-html t-html yesNoType" data-id="{{$question->id}}">
                                            ...
                                            @elseif ($question->question_type == 'trueFalseType')
                                                <div class="q-html t-html trueFalseType" data-id="{{$question->id}}">
                    ...
                    <div class="row mt-5">
                        <div class="col-md-2 form-group">
                            <button class="btn btn-warning w-100" type="submit">Save Changes</button>
                        </div>
                        <div class="col-md-2">
...
var checkboxToSelect = $('#includeProjectArray').val();

$('input.checkedArray[value="' + checkboxToSelect + '"]').prop('checked', true).trigger('change');
...
$('#includeProject').on('click', function(e){
    e.preventDefault();
...
document.getElementById('updateAssessment').addEventListener('submit', function(e) {
        e.preventDefault();
        var form = this;
        ...
        if(validator == false){
            swal({
                icon: 'error',
                iconColor: '#f1b82d',
                text: 'Please provide options for all choice type questions.',
                timer: 3000,
                type: "error",
                showConfirmButton: false,
            });
        ...
        if(question.length == 0){
            swal({
                icon: 'error',
                iconColor: '#f1b82d',
                text: 'Questions required. Add atlest one question.',
                timer: 3000,
                type: "error",
                showConfirmButton: false,
            });
        ...
        $.ajax({
            url: "{{ route('assessment.update', ['id' => $assessment->id]) }}",
            type: "POST",
            data: {
            ...
            success: function (response) {
                $(window).scrollTop(0);
                $('#successMessage').show().find('.alert').text(response.success);
